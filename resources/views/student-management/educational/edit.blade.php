...
<body>
    ...
    <form action="{{ route('educational.update', $history->educational_historyid) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <select name="studentid" id="studentid" required>
                ...
                @foreach ($students as $student)
                    <option value="{{ $student->studentid }}"
                        {{ old('studentid', $history->studentid) == $student->studentid ? 'selected' : '' }}>
        ...
        <p>
            ...
            <input type="text" id="institution_name" name="institution_name"
                value="{{ old('institution_name', $history->institution_name) }}" maxlength="100" required>
        </p>

        <p>
            <label for="institution_address">Alamat Institusi</label><br>
            <textarea id="institution_address" name="institution_address" required>{{ old('institution_address', $history->institution_address) }}</textarea>
        </p>

        <p>
            <label for="from_age_group">Kelompok Usia Asal</label><br>
            <input type="text" id="from_age_group" name="from_age_group"
                value="{{ old('from_age_group', $history->from_age_group) }}" maxlength="50" required>
        </p>

        <p>
            <label for="admitted_date">Tanggal Masuk</label><br>
            <input type="date" id="admitted_date" name="admitted_date"
                value="{{ old('admitted_date', $history->admitted_date) }}" required>
        </p>
        ...
        <p>
            ...
            <input type="text" id="admitted_age_group" name="admitted_age_group"
                value="{{ old('admitted_age_group', $history->admitted_age_group) }}" maxlength="50" required>
        </p>
